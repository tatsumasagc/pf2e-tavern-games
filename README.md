...
During the first game only, the selected deck owner is the first **Poppy/Dealer**. If that actor has an inventory item with the slug `marked-playing-cards`, their Deal cards panel offers a choice to use marked cards and select exactly two cards for their opening hand. Whether or not marked cards are available or selected, the module rolls a secret PF2E **Thievery** check with `action: "palm-an-object"` against the highest Perception DC among the other participants. The roll has no effect for a fair deal. If marked cards were used and the check fails or critically fails, every other participating character owner receives a private message: “You think [actor] is cheating.” A successful or critically successful cheating check produces no suspicion whisper. Only when the deck owner actually uses marked cards, their actor-owned player panel also gains a **Marked-card sight** section. It displays the text identity of every other player’s hand card and every unrevealed common card directly above its face-down card-back image. No card identity or cheating marker is written to the public board, so no other player receives this information.
