...
The source and releases are hosted at [tatsumasagc/pf2e-tavern-games](https://github.com/tatsumasagc/pf2e-tavern-games). Version 1.9.0 changes the Foundry package ID to `pf2e-tavern-games`, which cleanly separates its packs from the retired Poppy’s Prize namespace. Shut down Foundry, delete the former `Data/modules/poppys-prize/` directory, and extract `pf2e-tavern-games-v1.9.0.zip` into `Data/modules/`. The final path must be:

```text
Data/modules/pf2e-tavern-games/module.json
```

Start the world as a GM and enable **PF2e Tavern Games**. The first GM load copies any active table state, public board, and actor-owned player views from the former `poppys-prize` namespace into the new namespace. Disable the retired Poppy’s Prize module if it remains listed; do not enable both modules together.

Restart Foundry, open **Manage Modules** for a PF2E world, and enable **PF2e Tavern Games**. A GM can then select the anchor button from the Token controls, or run this macro:

```js
game.modules.get("pf2e-tavern-games")?.api.open();
```
...
Only when the active Poppy elects to use marked cards, their actor-owned player panel gains a **Marked-card sight** section. It has one labelled group for each other player’s hidden hand and a separate **Common pool** group. Every concealed card displays its text identity directly above its face-down card-back image. No card identity or marked-card marker is written to the public board, so no other player receives this information.

In each subsequent game at the same table, the new Poppy may use marked-card sight if their actor has an item with the `marked-playing-cards` slug. This later-game choice reveals only the concealed card identities; it does **not** allow the Poppy to select two opening cards, which remains restricted to the first game.
