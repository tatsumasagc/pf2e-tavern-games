...
assert.equal(new Set(golem.deck.map((card) => card.image)).size, 52, "Every Golem card should map to a distinct supplied dark-gold face.");
assert.ok(golem.deck.every((card) => /^assets\/golem-cards\/(clubs|diamonds|hearts|spades)-(02|03|04|05|06|07|08|09|10|ace|jack|queen|king)\.webp$/.test(card.image)), "Golem should use only mapped dark-gold standard-card faces.");
assert.ok(golem.deck.some((card) => card.image === "assets/golem-cards/spades-ace.webp" && card.displayLabel === "Ace of Spades"), "Golem should label mapped supplied cards with their visible standard suit.");
assert.ok(golem.deck.every((card) => !card.image.endsWith("back.webp") && !card.image.endsWith("joker.webp")), "The dark-gold back and joker must never enter Golem’s playable deck.");
