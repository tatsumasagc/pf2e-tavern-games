...
const golemAssetNames = readdirSync(golemCardDirectory);
...
assert.match(readmeSource, /golem_card_back\.png/, "The README should identify the supplied Golem card-back file.");
assert.match(readmeSource, /Flesh, Clay, Stone, and Iron/, "The README should identify all supplied numbered Golem material suits.");
...
assert.match(tavernEngineSource, /GOLEM_SUIT_ART/, "Golem should provide an explicit mapping from its logical suits to the supplied material-suit card assets.");
...
for (const suit of ["flesh", "clay", "stone", "iron"]) assert.match(tavernEngineSource, new RegExp(`file: "${suit}"`), `Golem should map the supplied ${suit} suit.`);
assert.match(tavernSource, /assets\/golem-cards\/golem_card_back\.png/, "Golem concealed cards should use the supplied golem_card_back.png image.");
assert.match(tavernSource, /renderGolemCardTable/, "The GM Golem table should render the supplied card faces.");
assert.equal(golemCards.length, 53, "Golem should bundle 52 supplied card faces plus golem_card_back.png.");
assert.equal(golemPlayableCards.length, 52, "Only the 52 supplied material-suit card faces should be playable in Golem.");
assert.ok(golemCards.includes("golem_card_back.png"), "Golem should include the supplied card back.");
assert.ok(golemPlayableCards.every((name) => /^(flesh|clay|stone|iron)_(01|02|03|04|05|06|07|08|09|10|11|12|13)\.png$/.test(name)), "Golem should include only the four supplied numbered material suits.");
assert.ok(!golemAssetNames.some((name) => name.endsWith(".webp") || name === "joker.webp"), "Retired dark-gold WebP artwork and the joker must be excluded from Golem assets.");
